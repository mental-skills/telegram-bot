...
def test_scenario_01_age_texts_are_used(engine_01: ScenarioEngine) -> None:
    ages: tuple[AgeGroup, ...] = ("6-8", "9-12", "13-16")
    texts = {age: engine_01.render("intro", age).text for age in ages}
    assert len(set(texts.values())) == 3
    assert "Подросток" in texts["13-16"]


def test_scenario_01_each_of_12_final_branches_reaches_advice(
    engine_01: ScenarioEngine,
) -> None:
    for first, second, expected_advice in FINAL_OPTIONS_01:
        first_transition = engine_01.transition("start_choice", first)
        second_transition = engine_01.transition(first_transition.to_node, second)
        outcome = engine_01.transition(second_transition.to_node, "continue")
        assert outcome.to_node == expected_advice
        advice_screen = engine_01.render(outcome.to_node, "9-12")
        assert advice_screen.node_type == "advice"
        assert advice_screen.quote


def test_scenario_01_common_part_reaches_completion(engine_01: ScenarioEngine) -> None:
    node = "summary_main"
    for _ in range(4):
        screen = engine_01.render(node, "9-12")
        if screen.node_id == "completion":
            assert screen.is_completion
            return
        transition = engine_01.transition(node, "continue")
...
def test_scenario_01_media_nodes_have_text_fallback(engine_01: ScenarioEngine) -> None:
    bundle = engine_01.bundle
    for node_id, node in bundle.scenario.nodes.items():
        if node.media:
            assert engine_01.render(node_id, "6-8").text
